...
<template>
    <Portal :handleClose="handleCloseModal" v-if="selectedExercise">
            ...
            <div class="workout-grid">
                ...
                <div class="workout-grid-row" v-for="(w, wIdx) in warmup" :key="wIdx">
                    <div class="grid-name">
                        <p>{{ w.name }}</p>
                        <button @click="() => {
                            console.log('clicked to open exercise description for', w.name);
                    ...
                    <p>{{ w.sets }}</p>
                    <p>{{ w.reps }}</p>
                    <input class="grid-weights" placeholder="14kgs" type="text" disabled/>
                </div>
                ...
                <div class="workout-grid-row" v-for="(w, wIdx) in workout" :key="wIdx">
                    ...
                    <p>{{ w.sets }}</p>
                    <p>{{ w.reps }}</p>
                    <input v-model="data[selectedWorkout][w.name]" class="grid-weights" placeholder="14kgs" type="text"/>
                </div>
            </div>
